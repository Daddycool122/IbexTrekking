    ...
    <div className={styles.whole_itonerary}>
        ...
        {days.map((item, index) => {
          return (
            <div
              className={
                selected_day == index
                  ? styles.indi_day_selected_hike
                  : styles.indi_day_hike
              }
              onClick={() => {
      ...
      {info.map((item, index) => {
        if (index == selected_day) {
          return (
            <div className={styles.indi_iti}>
              <div className={styles.title}>{item.title}</div>
              <div className={styles.para}>{item.para1}</div>
              <div className={styles.para}>{item.para2}</div>
              <div className={styles.img_Section}>
                <img loading="lazy" src={item.img} className={styles.img} alt={item.img ? item.img.split("/").pop().split("-").slice(0, -1).join(" ") : "Ibex Trekking"} />
              </div>
