...
export default function Form({ open_form, setOpen_form, name, price }) {
  ...
  const startPayment = async () => {
    ...
    const baseAmount = price * no_of_people;
    const gstAmount = (baseAmount * 5) / 100;
    ...
    try {
      await launchRazorpayPayment({
        ...
        onVerified: (result, bookingData) => {
          ...
          alert("✅ Payment Successful! Your adventure is booked.");
          setOpen_form(false);
        ...
        {no_of_people >= 1 && (
          <>
            <div className={styles.mobile_pay_summary}>
              <div>Subtotal</div>
              <div>₹{(price * no_of_people).toLocaleString()}</div>
            </div>
            <div className={styles.mobile_pay_summary}>
              <div>GST (5%)</div>
